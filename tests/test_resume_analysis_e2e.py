...
async def test_resume_paused_group_analyzes_only_pending_and_failed(tmp_path: Path) -> None:
    ...
    with patch("chatfilter.web.routers.groups.analysis._get_group_service", return_value=service):
        ...
        # Verify response (new architecture returns HTML partial with status 200)
        assert response.status_code == 200
        # Note: endpoint now returns TemplateResponse with group card HTML,
        # not empty response with HX-Trigger header
...
async def test_resume_concurrent_requests_return_409(tmp_path: Path) -> None:
    ...
    with patch("chatfilter.web.routers.groups.analysis._get_group_service", return_value=service):
        ...
        # Note: error responses return HTMLResponse with HX-Trigger (toast)
