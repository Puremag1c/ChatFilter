...
def _make_locale_dir(
    tmp_path: Path, locale: str, po_content: bytes, mo_content: bytes | None = None
) -> Path:
...
class TestCompileMoFallback:
    ...
    def test_corrupt_po_logs_warning(
        ...
        with caplog.at_level(logging.WARNING, logger="chatfilter.i18n.translations"):
            self._call_compile(tmp_path)
        assert any(
            "Failed to compile" in r.message or "compile" in r.message.lower()
    ...
    def test_corrupt_po_preserves_existing_mo(self, tmp_path: Path) -> None:
        ...
        self._call_compile(tmp_path)
        assert mo_path.read_bytes() == VALID_MO_SENTINEL, (
            "Existing .mo was overwritten despite corrupt .po"
        )
    ...
    def test_no_crash_when_babel_unavailable(self, tmp_path: Path) -> None:
        ...
        with (
            patch.object(mod, "LOCALES_DIR", tmp_path),
            patch.dict(
                "sys.modules", {"babel.messages.mofile": None, "babel.messages.pofile": None}
            ),
        ):
