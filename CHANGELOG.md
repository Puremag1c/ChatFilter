...
### Added
- AI пост-фильтр: после сбора и дедупликации AI проверяет каждый @ref на релевантность запросу и убирает явный мусор (работает для всех платформ, включая API-based tlgrm/tgstat)
...
### Added
- CI: деплой автоматически запускается при пуше тега v*
...
### Changed
- Query generator: AI теперь думает о тематике аудитории и генерирует 6-10 запросов по разным углам (музыка, студенты, субкультуры, локальные сообщества) вместо простого перефразирования
...
### Fixed
- Retry: ошибочные чаты сбрасываются в pending перед retry-пассом — счётчик "Обработано" снова бежит, видно прогресс retry
