...
class TelegramAPICheckStrategy:
    """Check connectivity by pinging Telegram API servers."""
    ...
    def __init__(
        ...
        """Initialize Telegram API check strategy.

        Args:
            hosts: List of Telegram API hosts to check
    ...
    async def check(self) -> NetworkStatus:
        """Check network by attempting TCP connection to Telegram servers."""
        start_time = datetime.now(UTC)
        ...
        try:
            for host in self.hosts:
                try:
                    ...
                    logger.debug(f"Telegram API check failed for {host}: {e}")
                    continue
            ...
            return NetworkStatus(
                is_online=False,
                last_check=datetime.now(UTC),
                error_message="Cannot reach Telegram API servers (network offline or Telegram blocked)",
                check_duration_ms=duration,
            )
