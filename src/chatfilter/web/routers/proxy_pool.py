...
def _get_user_id(request: Request) -> str:
    ...
    return get_session(request).get("user_id", "")
...
def _get_sessions_using_proxy(proxy_id: str, user_id: str) -> list[str]:
    ...
    Args:
        ...
        user_id: Owner of the sessions to check.

    Returns:
        List of session IDs that have this proxy configured.
    """
    sessions_using = []
    sessions_dir = get_settings().sessions_dir / user_id if user_id else get_settings().sessions_dir
